...
const EXAMPLES = [
  ...
  },
  {
    ...
    text: `This M&V plan follows the SEEC Energy Saving Measurement & Verification User Guide (Version 2, 2024) for a 10-year energy savings performance contract at a government university campus in Riyadh. The project targets 25% energy reduction through chiller plant optimization, building envelope improvements, and LED lighting retrofits across 14 buildings. IPMVP Option C is selected with the measurement boundary at the whole-facility level using existing Saudi Electricity Company (SEC) revenue meters. The baseline is constructed from 24 months of monthly billing data (1445-1446 H) to capture two full summer cooling cycles. A cooling degree day regression model using Riyadh regional weather data normalizes for the cooling-dominated load profile; heating degree days are excluded as negligible. Routine adjustments account for CDD variation between baseline and reporting periods. A non-routine adjustment provision addresses potential occupancy changes, as government buildings in the Kingdom are periodically repurposed between ministries. Operational verification per SEEC Section 4.5 will confirm that ECM installations meet design specifications before the reporting period begins. Savings are calculated monthly as the difference between baseline-adjusted predicted consumption and actual metered consumption. A 3rd-party M&V Agent, appointed per SEEC Section 2.3, will review this plan during preparation and independently verify annual saving reports. The M&V budget is set at 3% of estimated annual savings. Saving reports will be submitted in the format specified by the ESCO contract annex.`,
  },
];
